...
public class TelosWidgetProviderDark extends AppWidgetProvider {
    ...
    static void updateAppWidget(Context context, AppWidgetManager appWidgetManager, int appWidgetId) {
        RemoteViews views = new RemoteViews(context.getPackageName(), R.layout.widget_main_dark);
        ...
        // Set up the intent that starts the WidgetServiceDark
        Intent intent = new Intent(context, WidgetServiceDark.class);
        ...
        PendingIntent pendingAdd = PendingIntent.getActivity(context, 2, addIntent, PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_IMMUTABLE);
        ...
        PendingIntent pendingJournal = PendingIntent.getActivity(context, 3, journalIntent, PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_IMMUTABLE);
        views.setOnClickPendingIntent(R.id.btn_add_journal, pendingJournal);

        // Template for list item clicks
        Intent toggleIntent = new Intent(context, TelosWidgetProviderDark.class);
        toggleIntent.setAction(ACTION_TOGGLE_HABIT_DARK);
        toggleIntent.putExtra(AppWidgetManager.EXTRA_APPWIDGET_ID, appWidgetId);
        PendingIntent pendingToggle = PendingIntent.getBroadcast(context, 1, toggleIntent, PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_MUTABLE);
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (ACTION_TOGGLE_HABIT_DARK.equals(intent.getAction())) {
            String habitId = intent.getStringExtra("EXTRA_HABIT_ID");
            if (habitId != null) {
                // Toggle directly in SharedPreferences
    ...
    private void refreshAllWidgets(Context context) {
        ...
        for (int id : idsLight) {
            TelosWidgetProvider.updateAppWidget(context, mgr, id);
        }
